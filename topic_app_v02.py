...
if df_raw is not None:
    ...
    all_years_list = sorted(df_work["year"].unique().tolist())

    if "_topic_sel_years" not in st.session_state:
        st.session_state["_topic_sel_years"] = []

    _yc1, _yc2, _yc3 = st.columns([1.2, 1.2, 7])
    with _yc1:
        if st.button("✅ 전체 선택", use_container_width=True, key="topic_sel_all"):
            st.session_state["_topic_sel_years"] = [str(y) for y in all_years_list]
            st.rerun()
    with _yc2:
        if st.button("❌ 전체 해제", use_container_width=True, key="topic_sel_none"):
            st.session_state["_topic_sel_years"] = []
            st.rerun()

    sel_years_topic = st.multiselect(
        "분석할 연도를 선택하세요 (선택하지 않으면 전체 연도 분석)",
        options=[str(y) for y in all_years_list],
        default=st.session_state["_topic_sel_years"],
        key="topic_year_multiselect",
    )
    st.session_state["_topic_sel_years"] = sel_years_topic

    # 연도 필터 적용
    if sel_years_topic:
        sel_years_int = [int(y) for y in sel_years_topic]
        df_work = df_work[df_work["year"].isin(sel_years_int)].reset_index(drop=True)
        st.info(f"📅 선택된 연도: **{', '.join(sel_years_topic)}** &nbsp;|&nbsp; 분석 문서 수: **{len(df_work):,}**개")
    else:
        st.info(f"📅 전체 연도 분석 · 총 **{len(df_work):,}**개 문서 · 연도 범위: **{df_work['year'].min()}~{df_work['year'].max()}**")
